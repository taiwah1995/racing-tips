...
```
https://taiwah1995.github.io/racing-tips/?v=20260920q
```
...
- **馬膽WP投注簿**: heading `馬膽WP投注簿 *馬膽投注 獨贏$100 位置$300` with no title underline; monthly W / P ledger from `data/wp-bets.json` using fullwidth `｜` only (e.g. `W｜投注 $400｜贏 $1165｜回報 +191.3%`), with TOTAL as two lines (`TOTAL｜投注 $1600｜贏 $2635｜` then `☆ 本月盈利 $1035 ☆ (回報 +64.7%)`)
- **Day detail**:
  - 「🏆 全日重心馬匹數據表」— 場次｜首選｜次選｜三選｜冷腳
  - 「🏆 全日重心馬推介」— 每日 3 隻
- **Filters**: venue tabs + 賽事月份
- **Hash routes**: `#/` home · `#/meeting/<id>` day detail
- **Cache bust**: `APP_DATA_VERSION` in `js/app.js` (currently `20260920q`) versions data fetches and asset URLs
