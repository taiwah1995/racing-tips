...
```
https://taiwah1995.github.io/racing-tips/?v=20260920g
```
...
Example: `7 銀亮濠俠 (前) 6.6` — odds (`h.odds`) sit immediately after the closing parenthesis on the same compact line. When a race has official top-4, a place badge（冠／亞／季／殿）follows the odds on that same line.
...
**9/12 ST:** tip sheet is labeled 9月12日沙田; official results are from **13/09/2026 ST** (12/09/2026 ST had no LocalResults). Tip horse names match the 13/09 meeting.
...
- **Filters**: venue tabs (全部／沙田／快活谷) + month selector「賽事月份」(defaults to the current month only)
- **Hash routes**: `#/` home · `#/meeting/<id>` day detail
- **Cache bust**: `APP_DATA_VERSION` in `js/app.js` (currently `20260920g`) versions data fetches and asset URLs
